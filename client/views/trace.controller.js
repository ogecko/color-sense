...
Template.trace.onCreated(function() {
	const self = this;
	store.subscribe(self);
	self.keyboard = new Keypress.Listener();	// ensure this is destroyed on template removal
});

Template.trace.onDestroyed(function() {
	const self = this;
	self.keyboard.destroy();
});

Template.trace.onRendered(function () {
	const self = this;
	...
	self.autorun(function() {
		...
		if (path) {
			img = ImageMesh(path, 'imagePaint', film);
			film.scene.add(img);
